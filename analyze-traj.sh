...
if [ $gmxversion = 1 ] ; then alias trjconv='${GMX_PREFIX} trjconv' ; else alias trjconv='${GMX_PREFIX} trjconv'; fi
...
echo "------------------------------------------------------------------------"
echo
#read -p "W> Enter 3 or 4 letter residue name of the CHromophore in the system:" cro
...
if [[ ! -s rmsf_${TAG}.xvg || ! -s xaver_${TAG}.pdb  ]]; then
   ...
   mv rmsf_${TAG}.xvg bfac_${TAG}.pdb analysis/
fi
...
if [[ ! -s rmsd-xaver-prot.xvg ]]; then
   ...
   mv rmsd-xaver-prot.xvg xaver_${TAG}.pdb analysis/
fi
...
echo "          Protein + chromophore"
#read -p "W> Enter RMSD-cutoff for cluster analysis:" csize 
csize=0.18
echo "Solute Solute" | cluster -f $XTC -s $TPR -n rmsd.ndx -cutoff ${csize} -b 5000 -method gromos -o rmsd-clust-${csize} -g clusters-${csize} -cl clusters-${csize}.pdb -dist rmsd-dist-${csize} >& log
if [[ ! -s clusters-${csize}.log || ! -s clusters-${csize}.pdb  ]]; then
   echo "E> (g_cluster:0) Failed ... check 'log' "
   exit 13
else
   mv rmsd-clust-${csize}.xpm clusters-${csize}.log clusters-${csize}.pdb rmsd-dist-${csize}.xvg analysis/
fi
...
mv ${TAG}-prot-clust-fit.xtc 100ps.* prot.* rmsd.ndx analysis/
echo "M> Done"
